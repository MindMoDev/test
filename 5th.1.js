                    ...
                    if (meta && meta.err === null) {
                        ...
                        for (let i = 0; i < preBalances.length; i++) {
                            try {
                                ...
                                if (amountInUSD > 5) {  // Neue Bedingung hinzugefügt
                                            ...
                                            if (tokenMint !== 'So11111111111111111111111111111111111111112') {
                                                ...
                                                if (tokenTransferResult) {
                                                    ...
                                                    if (!recentlySentMessages.has(messageKey) ||
                                                        ...
                                                        //Filter für kleine Buys und kleine MCap
                                                        if (tokenTransferResult.marketCap.toLocaleString('en-US', { maximumFractionDigits: 2, minimumFractionDigits: 0 }) > 20000) {
                                                            tokenTransfers.push(tokenTransferMessage);
                                                            recentlySentMessages.set(messageKey, Date.now());
                                                        } else {
                                                            console.log(`Skipped: Market cap or Price too low`);
                                                        }
...
async function getTokenTransferMessageWithMetadata(tokenMint, amountReceived, fromAddress, signature) {
    ...
    try {
        ...
        const Telegram = await getTelegramLinkFromDexScreener(tokenMint) || "N/A";
        const CreatedAt = tokenDataStore[tokenMint]?.CreatedAt;
        ...
        if (totalSupply !== null && price !== null) {
            ...
            storeTokenData(tokenMint, marketCap, bagWorthUSD, Ticker, Name, Twitter, Telegram, CreatedAt);
            ...
            return { tokenTransferMessage, tokenSymbol, marketCap };
        } else {
            return null;
        }
    } catch (error) {
...
📈 <a href="https://dexscreener.com/solana/${tokenMint}">Chart</a>   ⏫ <a href="https://t.me/c/2163785746/${messageId}">Trending</a>
    
⚫ | <b>TROJAN TRENDING #${position}</b>
 <a href="https://t.me/solana_trojanbot?start=r-majolie814046">Start Trading on Trojan</a>`;
...
async function generateTopTrendingMessage() {
    ...
    let message = '<b>TOP TROJAN TRENDING</b> | <a href="https://t.me/trojantrendingsolananews/2">Disclaimer</a>\n\n';
